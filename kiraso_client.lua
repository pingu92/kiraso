...
local function check_override_ui(players, player_id)
	...
	if client_player then
		if client_player.kingdom == "rayyas_children:kingdoms:rayyas_children" then
			_radiant.res.apply_manifest("/kiraso/rayyas_children_selected/manifest.json")
		end
	end
end
...
radiant.events.listen(kiraso_client, 'radiant:init', function()
	trace_player_service()
	end)

return kiraso_client
